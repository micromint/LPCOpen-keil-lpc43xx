...
#define HID_SIO_MAX_DATA_LEN			1024	/*!< Maximum data size for each transfer */
...
#define HID_SIO_MAX_TRANSFER_LEN		HID_SIO_CALC_TRANSFER_LEN(HID_SIO_MAX_DATA_LEN)	/*!< Max transfer length for max data size */
#define HID_SIO_MAX_TRANSFER_BUFF_SZ	(HID_SIO_MAX_DATA_LEN + HID_SIO_PACKET_HEADER_SZ + HID_SIO_MAX_XFER_PARAM_SIZE) /*!< Max Transfer Buffer size on firmware */
#define HID_SIO_MAX_TRANSFER_BUFFERS	4		/*!< Maximum transfers allowed in processing queue */
...
/** HID_SIO responses. The response code below 0x10 should match with I2CM_STATUS codes. */
...
#define HID_SIO_RES_NAK             0x02		/*!< No device responded for given slave address. */
#define HID_SIO_RES_BUS_ERROR       0x03		/*!< I2C bus error */
#define HID_SIO_RES_SLAVE_NAK       0x04		/*!< NAK received after SLA+W or SLA+R */
#define HID_SIO_RES_ARBLOST         0x05		/*!< Arbitration lost */

#define HID_SIO_RES_TIMEOUT         0x10		/*!< Transaction timed out. */
#define HID_SIO_RES_INVALID_CMD     0x11		/*!< Invalid HID_I2C Request or Request not supported in this version. */
...
///** SPI pre/post delay after device selection - Not needed */
//#define HID_SPI_CONFIG_OPTION_SEL_DELAY(x)     ((x & 0xFF) << 8)
